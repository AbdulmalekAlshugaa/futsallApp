...
const CreateCenter = memo(({ close }) => {
  const router = useRouter()
  const [center, setCenter] = useState({ name: '', phone: '', address: '' })
  ...
  const addCenter = async (e) => {
    ...
    if (!center.name || !center.phone || !center.address) {
      cogoToast.error('Please fill all the requirement')
      setIsAddingCenter(false)
      return
    }
    const loader = cogoToast.loading('Adding centers', { hideAfter: 0 })
    try {
      await axios.post('/api/user/createCenter', {
        ...center
      })
      router.reload()
    } catch (error) {
            ...
            <div className='mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full'>
              ...
              <div className='mt-2'>
                <div>
                  <form>
                    <div>
                      <input onChange={handleChange} name='name' className='mb-2 appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:shadow-outline-blue focus:border-blue-300 focus:z-10 sm:text-sm sm:leading-5' placeholder='Enter center name' />
                      <input onChange={handleChange} name='phone' className='mb-2 appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:shadow-outline-blue focus:border-blue-300 focus:z-10 sm:text-sm sm:leading-5' placeholder='Enter center phone' />
                      <input onChange={handleChange} name='address' className='mb-2 appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:shadow-outline-blue focus:border-blue-300 focus:z-10 sm:text-sm sm:leading-5' placeholder='Enter center address' />
                    </div>
                  </form>
                </div>
              </div>
