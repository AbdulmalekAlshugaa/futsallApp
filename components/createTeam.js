  ...
  const handleCreateTeam = async () => {
    try {
      ...
      const res = await axios.post('/api/user/createTeam', {
        listOfPlayers: myTeam, bookingId: Router.router.query.bookingId
      })
      loader.hide()
  ...
  return (
    <div>
      ...
      <div className='max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8  h-screen pt-4' style={{ backgroundColor: '#e5e5e5' }}>
        <div>
          <div>
          Number Of players Selected: {myTeam.length}
          </div>
          <div>
            <Button variantColor='blue' onClick={handleCreateTeam}>Create Team</Button>
          </div>
        </div>
            ...
            <Box key={p.email} rounded='md' p={4} w='100%' bg='#fff'>
              ...
              <Box>
                {myTeam.includes(p.email) ? (
                  <Button onClick={() => setMyTeam(myTeam.filter(pp => pp !== p.email))} variantColor='red'>Remove</Button>
                ) : (
                  <Button onClick={() => setMyTeam([...myTeam, p.email])}>Request</Button>
                )}
              </Box>
            </Box>
          ))}
